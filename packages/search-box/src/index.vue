...
import { reactive, getCurrentInstance, watch, nextTick, onMounted, computed, onBeforeUnmount } from 'vue-demi'
...
const props = defineProps({
  modelValue: {
    type: <ISearchBoxTag[]>Array,
    default() {
      return []
    }
  },
  items: {
    type: <ISearchBoxItem[]>Array,
    default: () => []
  },
  emptyPlaceholder: {
    type: String,
  ...
  potentialOptions: {
    type: <ISearchBoxMatchOptions>Object,
    default() {
      return null
    }
  },
...
const isSaas = computed(() => props.mode === 'saas')
...
<template>
  <TinyConfigProvider :design="isSaas ? designSaasConfig : {}">
    <div class="tvp-search-box" :class="{'tvp-search-box__saas': isSaas}" @click.stop="showPopover(state, false)">
