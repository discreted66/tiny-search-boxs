...
const parseDate = (str) => {
  for (let i = 0, len = dateParsers.length; i < len; i++) {
    const parser = dateParsers[i]
    const regex = parser[0] as RegExp
    const parserFunc = parser[1] as (m: any) => Date
...
export const toDate = (value, dateFormat, minDate) => {
  ...
  if (minDate) {
    const min = (minDate && toDate(minDate, dateFormat, undefined)) || new Date(1, 1, 1, 0, 0, 0)
  ...
  if (isDate(date)) {
    if (typeof dateFormat === 'string') {
      ...
      Object.keys(o).forEach((k) => {
        ...
        if (k && m && m.length) {
          dateFormat = dateFormat.replace(m[0], k === 'Z{1,1}' ? o[k] : fillChar(o[k].toString(), m[0].length, false, '0'))
        }
      })
    ...
    const dateValue = toDate(date, dateFormat, afterFormat)
...
export const toDateStr = (date, dateFormat, timezone) => {
  if (date && isNumeric(timezone)) {
    ...
    date = getDateWithNewTimezone(isDate(date) ? date : new Date(toDate(date, dateFormat, undefined)), 0, timezone)
  }
...
export const getStrTimezone = (value) => {
  ...
  if (match) {
    ...
    value = minoffset * Number(`${match[1]}1`)
  }
